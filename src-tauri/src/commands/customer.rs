...
#[tauri::command]
pub async fn customer_get(state: State<'_, AppState>, id: i64) -> Result<serde_json::Value, AppError> {
    let db = state.db.lock().map_err(|_| AppError::DatabaseLock)?;
    db.customer_get(id)
}
#[tauri::command]
...
#[tauri::command]
pub async fn customer_update(state: State<'_, AppState>, id: i64, data: serde_json::Value, user_id: i64) -> Result<(), AppError> {
    let db = state.db.lock().map_err(|_| AppError::DatabaseLock)?;
...
#[tauri::command]
pub async fn doctor_create(state: State<'_, AppState>, data: DoctorCreateInput, user_id: i64) -> Result<i64, AppError> {
    let db = state.db.lock().map_err(|_| AppError::DatabaseLock)?;
    db.doctor_create(&data, user_id)
}
#[tauri::command]
pub async fn doctor_update(state: State<'_, AppState>, id: i64, data: serde_json::Value, user_id: i64) -> Result<(), AppError> {
    let db = state.db.lock().map_err(|_| AppError::DatabaseLock)?;
