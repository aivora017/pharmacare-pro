...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        setLoading(true)
        const [summary, lowStock, expiryRows, briefing] = await Promise.all([
          billingService.getTodaySummary() as Promise<{
            total_revenue: number
            bill_count: number
          }>,
          inventoryService.getLowStock(),
          inventoryService.getExpiryList(30),
          aiService.getMorningBriefing(),
        ])

        setTodaySales(summary.total_revenue ?? 0)
        setBillCount(summary.bill_count ?? 0)
        setLowStockCount(lowStock.length)
        setExpiryCount(expiryRows.length)
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          >
            ...
            <span className={`text-xs ${b.primary ? 'text-blue-200' : 'text-slate-400'}`}>
              {b.sub}
            </span>
          </button>
        ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            sub: 'high risk batches',
            color: expiryCount > 0 ? 'red' : 'green',
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <div
                className={`w-8 h-8 bg-${s.color}-50 rounded-lg flex items-center justify-center text-${s.color}-600`}
              >
                {s.icon}
              </div>
            </div>
            <p className="text-2xl font-bold text-slate-800">{s.value}</p>
            <p className="text-xs text-slate-400 mt-0.5">{loading ? 'Loading...' : s.sub}</p>
          </div>
        ))}
      </div>
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="space-y-2">
          {briefingActions.length === 0 ? (
            <p className="text-sm text-slate-500">No briefing items.</p>
          ) : (
            briefingActions.map((item, index) => (
              <button
                ...
                className="w-full text-left rounded-lg border border-slate-200 bg-slate-50 hover:bg-slate-100 px-3 py-2"
              >
