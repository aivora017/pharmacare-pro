...
export function Sidebar() {
  ...
  const prefetchRoute = useCallback((path: string) => {
    ...
    void loader.catch(() => {
      prefetchedRoutesRef.current.delete(path)
    })
  }, [])
  ...
  return (
    <aside
      className={clsx(
        'flex flex-col h-screen bg-slate-900 text-white flex-shrink-0 transition-all duration-200',
        collapsed ? 'w-16' : 'w-56'
      )}
    >
          ...
          <div key={g.section} className="mb-3">
            ...
            {g.items.map((item) => (
              ...
              >
                ...
                {!collapsed && 'shortcut' in item && (
                  <kbd className="text-xs text-slate-500 font-mono">
